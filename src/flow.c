...
void rist_delete_flow(struct rist_receiver *ctx, struct rist_flow *f)
{
	...
	if (ctx->flow_cb.delete_cb) {
		ctx->flow_cb.delete_cb(ctx->flow_cb.arg, flow_id);
	}
}
...
static struct rist_flow *create_flow(struct rist_receiver *ctx, uint32_t flow_id)
{
	...
	if (ctx->flow_cb.create_cb) {
		ctx->flow_cb.create_cb(ctx->flow_cb.arg, flow_id);
	}
...
int rist_receiver_associate_flow(struct rist_peer *p, uint32_t flow_id)
{
	...
	f->session_timeout = p->session_timeout;  // RISTX
	...
	if (f->recovery_buffer_ticks_min < p->recovery_buffer_ticks_min) {  // RISTX
		f->recovery_buffer_ticks_min = p->recovery_buffer_ticks_min;
	}

	rist_log_priv(&ctx->common, RIST_LOG_INFO, "FLOW #%"PRIu32" session_timeout set to %"PRIu64" from %"PRIu64"\n",
		flow_id, f->session_timeout, p->session_timeout);
	...
	if (ret == 1) {
		...
		if (ctx->flow_cb.assoc_cb) {
			ctx->flow_cb.assoc_cb(ctx->flow_cb.arg, flow_id, p);
		}
	}
