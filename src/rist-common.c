...
static void init_peer_settings(struct rist_peer *peer)
{
	if (peer->receiver_mode) {
		...
		peer->recovery_buffer_ticks = peer->config.recovery_length_max * RIST_CLOCK;  // RISTX
		peer->recovery_buffer_ticks_min = peer->config.recovery_length_min * RIST_CLOCK;  // RISTX
	...
	while (atomic_load_explicit(&f->receiver_queue_size, memory_order_acquire) > 0) {
		...
		if (b) {
			if (b->type == RIST_PAYLOAD_TYPE_DATA_RAW) {
				...
				else if (b->packet_time + f->recovery_buffer_ticks_min >= now) {  // RISTX
