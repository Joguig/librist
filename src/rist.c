...
int ristX_flow_callback_set(struct rist_ctx *ctx, 
		void (*create_cb)(void *arg, uint32_t flow_id),
		void (*assoc_cb)(void *arg, uint32_t flow_id, struct rist_peer *peer),
		void (*delete_cb)(void *arg, uint32_t flow_id),
		void *arg) {
	if (RIST_UNLIKELY(!ctx))
	{
		rist_log_priv3(RIST_LOG_ERROR, "ristX_flow_callback_set call with null ctx!\n");
		return -1;
	}
	if (ctx->mode != RIST_RECEIVER_MODE || !ctx->receiver_ctx) {
		rist_log_priv3(RIST_LOG_ERROR, "ristX_flow_callback_set call on non receiver\n");
		return -1;
	}
	ctx->receiver_ctx->flow_cb.create_cb = create_cb;
	ctx->receiver_ctx->flow_cb.assoc_cb = assoc_cb;
	ctx->receiver_ctx->flow_cb.delete_cb = delete_cb;
	ctx->receiver_ctx->flow_cb.arg = arg;
	return 0;
}

const struct sockaddr_storage* ristX_peer_get_sockaddr_storage(const struct rist_peer *peer) {
	return &peer->u.storage;
}

uint32_t ristX_peer_get_flow_id(const struct rist_peer *peer) {
	// XXX: is zero a valid flow?
	if (!peer->flow)
		return 0;
	return peer->flow->flow_id;
}

uint16_t ristX_peer_get_local_port(const struct rist_peer *peer) {
