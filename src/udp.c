...
void rist_create_socket(struct rist_peer *peer)
{
	...
	// RISTX: replace checking local port here with listening to try to bind an OS specified port
	if (peer->listening) {
		const char* host;
		uint16_t port;
		...
		// RISTX: Update local port with bound port
		if (peer->local_port == 0) {
			struct sockaddr_storage peer_sockaddr;
			socklen_t peer_socklen = sizeof(peer_sockaddr);
			peer_sockaddr.ss_family = AF_UNSPEC;
			int ret_sockname = getsockname(peer->sd, (struct sockaddr *)&peer_sockaddr, &peer_socklen);
			if (ret_sockname == 0) {
				switch (peer_sockaddr.ss_family) {
					case AF_INET:
						peer->u.inaddr.sin_port = ((struct sockaddr_in *)&peer_sockaddr)->sin_port;
						peer->local_port = ntohs(peer->u.inaddr.sin_port);
						break;
					case AF_INET6:
						peer->u.inaddr6.sin6_port = ((struct sockaddr_in6 *)&peer_sockaddr)->sin6_port;
						peer->local_port = ntohs(peer->u.inaddr6.sin6_port);
						break;
					default:
						rist_log_priv(get_cctx(peer), RIST_LOG_ERROR, "Unknown socket address family %u\n", (unsigned)(peer_sockaddr.ss_family));
				}
				if (peer->local_port != 0) {
					rist_log_priv(get_cctx(peer), RIST_LOG_INFO, "URL Listening socket (socket# %d) bound to port %hu\n", peer->sd, peer->local_port);
				}
			} else {
				rist_log_priv(get_cctx(peer), RIST_LOG_ERROR, "Failed to getsockname (errno=%d) %s\n", errno, strerror(errno));
			}
		}
	}
	else {
...
static inline void rist_rtcp_write_rr(uint8_t *buf, int *offset, const struct rist_peer *peer)
{
	...
	*offset += sizeof(struct rist_rtcp_rr_pkt);
	...
	rr->highest_seq = 0;  // XXX
	if (peer->flow)
		rr->highest_seq = htobe32(peer->flow->last_seq_found);
	rr->jitter = 0;
