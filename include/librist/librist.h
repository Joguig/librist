...
/**
 * @brief Set flow lifecycle callbacks
 */
RIST_API int ristX_flow_callback_set(struct rist_ctx *ctx,
...
RIST_API const struct sockaddr_storage* ristX_peer_get_sockaddr_storage(const struct rist_peer *peer);

RIST_API uint32_t ristX_peer_get_flow_id(const struct rist_peer *peer);

RIST_API uint16_t ristX_peer_get_local_port(const struct rist_peer *peer);
#ifdef __cplusplus
}
#endif
